...
const { spendingByMerchant, spendingByCategory } = useAccountsData();

const activeTab = ref<"category" | "merchant">("category");
...
const maxMerchantTotal = computed(() => spendingByMerchant.value[0]?.total ?? 1);
const maxCategoryTotal = computed(() => spendingByCategory.value[0]?.total ?? 1);
  ...
  <div class="space-y-4">
    ...
    <div class="bg-bg-card rounded-xl border border-border-light p-5">
      ...
      <div class="flex gap-6 mb-4">
        ...
        <div>
          <p class="text-xs text-text-muted uppercase tracking-wider mb-1">Categories</p>
      ...
      <!-- Tab switcher -->
      <div class="flex gap-1 bg-bg-input rounded-lg p-0.5">
        <button
          @click="activeTab = 'category'; expanded = null"
          class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-colors"
          :class="activeTab === 'category' ? 'bg-white shadow-sm text-text-primary' : 'text-text-muted hover:text-text-secondary'"
        >
          By Category
        </button>
        <button
          @click="activeTab = 'merchant'; expanded = null"
          class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-colors"
          :class="activeTab === 'merchant' ? 'bg-white shadow-sm text-text-primary' : 'text-text-muted hover:text-text-secondary'"
        >
          By Merchant
        </button>
      </div>
    ...
    <!-- Category bars -->
    <div v-if="activeTab === 'category'" class="bg-bg-card rounded-xl border border-border-light overflow-hidden">
      <div
      ...
      >
          ...
          <div class="flex items-center justify-between mb-1.5">
            <div class="flex items-center gap-2">
              <span
                ...
                :class="CATEGORY_COLORS[cat.category]"
              >
                {{ CATEGORY_LABELS[cat.category] }}
              </span>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <Badge variant="muted">{{ cat.count }}x</Badge>
        ...
        <!-- Expanded: transactions in this category -->
        <div
          v-if="expanded === cat.category"
          class="px-5 py-2 bg-bg-input/30 border-t border-border-light"
        >
          <div
            v-for="tx in cat.transactions"
            :key="tx.id"
      ...
      <div v-if="spendingByCategory.length === 0" class="p-8 text-center text-text-muted text-sm">
        No spending data in the last 3 months.
      </div>
    </div>

    <!-- Merchant bars -->
    <div v-if="activeTab === 'merchant'" class="bg-bg-card rounded-xl border border-border-light overflow-hidden">
      <div
      ...
      >
        ...
        >
          ...
          <div class="h-2 bg-bg-input rounded-full overflow-hidden">
            <div
              class="h-full bg-danger/40 rounded-full transition-all"
              :style="{ width: `${(merchant.total / maxMerchantTotal) * 100}%` }"
            />
          </div>
        </button>
