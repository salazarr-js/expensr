...
src/
  ...
  views/
    ...
    PeopleView.vue      # debts by person + cross-reference with transactions_long
    SpendingView.vue    # merchant spending breakdown with expandable bars
```

## Data

`public/all_documents.json` contains 3 sections:
...
| `transactions_long` | 2025.xlsx + 2026.xlsx (manual) | 2,277 txns | numbers | `YYYY-MM-DD` |

MercadoPago and Galicia are the primary data. `transactions_long` is cross-reference only (manual expense registers).
...
- **Categories:** `transfer_in`, `transfer_out`, `payment`, `purchase`, `debt_debit`, `refund`, `income`, `service`, `tax`, `extraction`, `other`.
