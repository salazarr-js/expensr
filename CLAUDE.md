...
packages/
  ...
  web/                 # Vue 3 SPA + Cloudflare deployment seat
    ...
    src/components/    # Custom components (IconPicker, ColorPicker, AlertDialog, AccountFormModal, CategoryFormModal, PersonFormModal, RecordFormModal, QuickRecordModal, TransferFormModal, DateRangePicker, BalancesModal, BalanceFormModal, MonthPicker)
...
Tables: accounts, account_balances, categories, tags, people, records, record_people (junction), parse_logs, keyword_mappings, draft_records. Factory: `createDb(d1: D1Database)` returns typed Drizzle instance.
...
- **Monthly balances**: `account_balances` table — one balance per account per month (bank says X on date Y). Balance computed as: latest monthly balance + records after balance date. Auto-computed initial (balance + records before) and projected end (balance + records after). Gap = projected end vs next month's balance. BalancesModal with timeline + BalanceFormModal with MonthPicker + UCalendar date picker (constrained to month). Auto-reconciliation suggestion when adding past months. Badge on account cards: "Set balance" / synced / off by N.
- **Account balance**: computed from latest monthly balance + records since balance date. Falls back to pure cash flow (sum of all records) when no balance is set. Incoming transfers count as positive. Transfers have `mySpend = 0` (not spending).
